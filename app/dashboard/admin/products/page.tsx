    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-slate-600 mt-1">Monitor and manage all products listed on the platform.</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        {loading ? (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200 text-xs uppercase tracking-wider text-slate-500 font-semibold">
                  ...
                  <th className="p-4">Factory ID</th>
                ...
                {products.map((product) => (
                  <tr key={product.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="p-4">
                      ...
                      <p className="text-xs text-slate-500">MOQ: {product.moq}</p>
                    </td>
                    <td className="p-4 text-right">
                      <div className="flex justify-end gap-2">
                        <Link href={`/products`} className="p-1.5 text-slate-400 hover:text-blue-600 transition-colors rounded-md hover:bg-blue-50" title="View Product">
                          <Eye className="w-4 h-4" />
                        </Link>
                        <button onClick={() => handleDeleteProduct(product.id)} className="p-1.5 text-slate-400 hover:text-red-600 transition-colors rounded-md hover:bg-red-50" title="Delete Product">
          ...
          <div className="p-12 text-center">
            ...
            <p className="text-slate-500">There are currently no products listed on the platform.</p>
          </div>
        )}
      </div>
    </div>
