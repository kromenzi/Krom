    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-slate-600 mt-1">Monitor all Requests for Quotation across the platform.</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
          <div className="relative w-full max-w-md">
            ...
            <input 
              type="text" 
              placeholder="Search RFQs by title, ID, or category..." 
        ...
        {loading ? (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200 text-xs uppercase tracking-wider text-slate-500 font-semibold">
                  ...
                  <th className="p-4">Buyer / Factory</th>
                  ...
                  <tr key={rfq.id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="p-4">
                      <div>
                        ...
                        <p className="text-xs text-slate-500">ID: {rfq.id.substring(0, 8)} | {rfq.category}</p>
                      </div>
                    </td>
                    <td className="p-4">
                      <p className="text-xs text-slate-600 font-mono">B: {rfq.buyerId.substring(0, 8)}</p>
                      <p className="text-xs text-slate-600 font-mono">F: {rfq.factoryId === 'general' ? 'General' : rfq.factoryId.substring(0, 8)}</p>
                    </td>
                    <td className="p-4">
                    ...
                    <td className="p-4 text-right">
                      <div className="flex justify-end gap-2">
                        <Link href={`/dashboard/rfqs/${rfq.id}`} className="p-1.5 text-slate-400 hover:text-blue-600 transition-colors rounded-md hover:bg-blue-50" title="View RFQ">
                          <Eye className="w-4 h-4" />
                        </Link>
                        <button onClick={() => handleDeleteRfq(rfq.id)} className="p-1.5 text-slate-400 hover:text-red-600 transition-colors rounded-md hover:bg-red-50" title="Delete RFQ">
          ...
          <div className="p-12 text-center">
            ...
            <p className="text-slate-500">There are currently no RFQs on the platform.</p>
          </div>
        )}
      </div>
    </div>
